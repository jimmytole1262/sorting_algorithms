...
/**
 * merge_sub - Sort a subarray of iedntegers.
 * @sub: A subarray of an array of integers to sort.
 * @buff: A buffer to store the sort subay.
 ...
 * @front: The front index of the subay.
 * @back: The back index of the subay.
 */
void merge_sort_recursive(int *sub, int *buff, size_t front, size_t back)
{
	...
	if (back - front > 1)
	{
		...
		merge_sub(sub, buff, front, mid, back);
	}
}
